...
const withRetry = async <T>(
  ...
  try {
    ...
    const isPermissionDenied = errorMsg.includes("permission denied") || errorMsg.includes("403") || errorMsg.includes("unauthorized");
    
    if (isPermissionDenied) {
      console.error("[Neural] Permission Denied (403). This usually means the API key is invalid, expired, or doesn't have access to the model. Stopping retries.");
      throw new Error("Gemini API Permission Denied: Your API key may be invalid or restricted. Please check your Engine settings or ensure you are in a supported region.");
    }
